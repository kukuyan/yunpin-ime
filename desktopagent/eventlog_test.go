...
func TestEventLogWritesOneRedactedLinePerEvent(t *testing.T) {
	...
	log.Write(RunEvent{Code: "sync_complete", Successful: true, Summary: SyncSummary{
		Rounds: 1, Uploaded: 2, Downloaded: 3, Cursor: 44,
	}})
	log.Write(RunEvent{Code: "sync_failed"})
	...
	if first.Code != "sync_complete" || !first.Successful || first.Summary.Cursor != 44 {
...
func TestEventLogCarriesNoFreeText(t *testing.T) {
	log, paths := newTestEventLog(t, eventLogMaxBytes)
	log.Write(RunEvent{Code: "sync_failed", Summary: SyncSummary{Rounds: 1}})
	...
	for name, value := range decoded {
		if name == "time" || name == "code" || name == "successful" || name == "summary" {
			continue
		}
...
func TestEventLogRotatesAndStaysBounded(t *testing.T) {
	...
	for index := 0; index < 200; index++ {
		log.Write(RunEvent{Code: "sync_complete", Successful: true, Summary: SyncSummary{Rounds: index}})
	}
...
func TestEventLogWriteAfterCloseIsDropped(t *testing.T) {
	...
	log.Write(RunEvent{Code: "sync_complete"})
	content, err := os.ReadFile(EventLogPath(paths))
	if err != nil {
		t.Fatalf("read log: %v", err)
	}
