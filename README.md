...
发行版自动纠错仍默认关闭，不发送输入内容，也没有 NearSearch 或纠错模型
回退。桌面预览随包携带按摘要锁定的完整 Wanxiang octagram grammar，在
librime 进程内、完全离线地参与整句上下文重排；macOS 与 Windows 使用同一
份模型摘要。这不会重新开启拼写纠错。后续若试验独立模型 sidecar，仍须可选、
默认关闭、不得联网并具有严格超时；超时、崩溃或结果校验失败时只保留普通
精确候选路径。中英混输仍只是下一版研究方向，当前预览不宣称已实现。
