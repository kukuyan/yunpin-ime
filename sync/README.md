...
`POST /v1/sync` accepts at most 256 upload envelopes and a 1 MiB JSON body. Canonical plaintext is capped at 512 KiB, so decoded envelope ciphertext is capped at 524816 bytes on both client and relay. It returns `accepted_sequences`, explicit `rejected_sequences`, downloaded envelopes, `next_cursor`, `has_more`, and `current_key_epoch`. Each download page is bounded by both 256 records and 524816 cumulative decoded ciphertext bytes; `has_more` remains true when either limit cuts the page. `ack_cursor` is monotonic, cannot exceed the request cursor, and records the last fully applied relay cursor for that device.

The relay compacts only history that every non-revoked device has acknowledged and that is older than 30 days. It retains the newest envelope for every `(device_id, object_id)` pair, so a cursor-zero device can reconstruct the current join of the full phrase CRDT states and every writer retains its global sequence/hash-chain head. Revoked devices are excluded from the acknowledgement watermark; an account with no active devices deletes nothing. A sync transaction deletes at most 1024 rows, preventing routine sync latency from scaling with old history. This rule is specific to protocol v1's complete monotone phrase-state payload: any future payload type must define an equally safe opaque frontier or use a signed encrypted checkpoint before it becomes eligible for relay compaction.
