...
class ShadowCandidate : public Candidate {
 public:
  ...
  string preedit() const override { return item_->preedit(); }
...
inline an<Candidate> Candidate::GetGenuineCandidate(
    const an<Candidate>& candidate) {
  const auto shadow = std::dynamic_pointer_cast<ShadowCandidate>(candidate);
  return shadow ? shadow->item() : candidate;
}
}  // namespace rime
