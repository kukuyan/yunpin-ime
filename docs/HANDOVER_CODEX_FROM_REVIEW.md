...
R0W 网络曾恢复并完成只读快照/转换；安装新 binary 前仍须重新验证实时
SSH/Tailnet 数据面，不能从历史在线状态推定当前可达。当前转换结果为 94,382
条合并词，能放入新的 100k 上限；但完整 TSV 仍只在
...
本机另有一份在原转换上增加 2 条显式手动置顶的 94,384 条派生快照；原
94,382 条文件保持不变。运行时把其中 149 条非标准搜狗码放入独立 private
exact-only 索引，只有完整代码相等时才召回，前缀、简拼、模糊音和纠错均不可
访问。两份文件仍未部署，且都不进入 Git。
